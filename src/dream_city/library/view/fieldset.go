...
func (f *FieldSet) InputTag(option string) template.HTML{
	return InputTag(f, option)
}
...
func initCommonField() {
	...
	datetimeFunc := func(fSet *FieldSet) {
		fSet.Field = func() template.HTML {
			...
			tval := beego.Date(t, conf.DateTimeFormat)
			if tval == "0001-01-01 00:00:00" {
				t = time.Now()
			}
			if fSet.Type == "date" {
				tval = beego.Date(t, conf.DateFormat)
			}
			return template.HTML(fmt.Sprintf(
					...
					if v.CanInterface() {
						if vu, ok := v.Interface().([][]string); ok {
							...
							if val.Kind() == reflect.Slice {
								...
								for i := 0; i < val.Len(); i++ {
									vs = append(vs, tools.ToStr(val.Index(i).Interface()))
								}
							}
							...
							for _, parts := range vu {
								...
								} else if tools.ToStr(fSet.Value) == v {
									selected = ` selected="selected"`
								}
	...
	for i := 0; i < elm.NumField(); i++ {
		...
		case reflect.Struct:
			if fT.Type.String() == "time.Time" {
				if len(value) > 10 {
					t, err := beego.DateParse(value, conf.DateTimeFormat)
					if err != nil {
						continue
					}
					f.Set(reflect.ValueOf(t))
				} else {
					t, err := beego.DateParse(value, conf.DateFormat)
					if err != nil {
						continue
					}
					f.Set(reflect.ValueOf(t))
...
func FormChanges(base interface{}, modified interface{}, skips ...string) (fields []string) {
	...
	for i := 0; i < elmMod.NumField(); i++ {
		...
		if tools.ToStr(modF.Interface()) != tools.ToStr(f.Interface()) {
